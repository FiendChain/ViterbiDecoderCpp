...
class ViterbiDecoder_ARM_u16: public ViterbiDecoder_Core<constraint_length, code_rate, uint16_t, int16_t, uint16_t>
{
private:
...
private:
    ...
    void bfly(const int16_t* symbols, uint16_t* decision, uint16_t* old_metric, uint16_t* new_metric) 
    {
        ...
        for (size_t curr_state = 0u; curr_state < m128_width_branch_table; curr_state++) {
            ...
            const uint16_t* d0 = reinterpret_cast<const uint16_t*>(&decision0);
            const uint16_t* d1 = reinterpret_cast<const uint16_t*>(&decision1);
            for (size_t i = 0; i < 8; i++) {
                const size_t j = i << 1;
                const uint16_t v = ((d0[i] & 0b1) | (d1[i] & 0b10)) << j;
                decision[curr_state] |= v;
            }
        }
    }
    ...
    void renormalise(uint16_t* metric) {
        ...
        // TODO: Use intrinsics
        /* adjustv = vminq_u16(adjustv, _mm_srli_si128(adjustv, 8)); */
        /* adjustv = vminq_u16(adjustv, _mm_srli_si128(adjustv, 4)); */
        /* adjustv = vminq_u16(adjustv, _mm_srli_si128(adjustv, 2)); */
        /* const uint16_t min = vduph_laneq_u16(adjustv, 0); */
