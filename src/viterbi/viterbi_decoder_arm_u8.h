...
class ViterbiDecoder_ARM_u8: public ViterbiDecoder_Core<constraint_length, code_rate, uint8_t, int8_t, uint32_t>
{
private:
...
private:
    ...
    void bfly(const int8_t* symbols, uint32_t* decision, uint8_t* old_metric, uint8_t* new_metric) 
    {
        ...
        for (size_t curr_state = 0u; curr_state < m128_width_branch_table; curr_state++) {
            ...
            // Pack decision bits
            // TODO: Use intrinsics
            const uint8_t* d0 = reinterpret_cast<const uint8_t*>(&decision0);
            const uint8_t* d1 = reinterpret_cast<const uint8_t*>(&decision1);
            for (size_t i = 0; i < 16; i++) {
                const size_t j = i << 1;
                const uint32_t v = ((d0[i] & 0b1) | (d1[i] & 0b10)) << j;
